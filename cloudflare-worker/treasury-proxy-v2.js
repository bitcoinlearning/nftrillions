/**
 * Cloudflare Worker: Treasury API Proxy with 12-hour caching (v2)
 * Optimized with fetch timeouts and better error handling
 */
...
async function handleRequest(request) {
  ...
  // Try TreasuryDirect first (more reliable for Workers)
  let debtData = await fetchFromTreasuryDirect();
  
  // Fallback to FiscalData if TreasuryDirect fails
  if (!debtData) {
    console.log('TreasuryDirect failed, trying FiscalData');
    debtData = await fetchFromFiscalData();
  }
  ...
  if (!debtData) {
    return new Response(JSON.stringify({
      ...
      amount: 37889756572760,
      formatted: '$37,889,756,572,760',
      cached: false,
      cacheAge: 0,
      source: 'fallback'
    }), {
...
async function fetchFromTreasuryDirect() {
  try {
    ...
    const response = await fetchWithTimeout(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; NFTrillions/1.0)',
    ...
    try {
      ...
      if (json.totalDebt) {
        const amount = parseFloat(json.totalDebt);
        return {
          amount: amount,
    ...
    console.error('TreasuryDirect fetch error:', error.message);
    return null;
  }
}
...
async function fetchFromFiscalData() {
  try {
    ...
    console.error('FiscalData fetch error:', error.message);
    return null;
  }
}
